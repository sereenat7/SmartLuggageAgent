...
// Vehicle type → maximum carrying capacity in kg
...
const parseWeightKg = (value) => {
  ...
  // For ranges like "10-20 kg", use the upper bound so capacity checks stay safe.
  if (lower.includes('-')) {
...
const getBestAgentIdForBooking = async (bookingId, options = {}) => {
  ...
  const ranked = greedyRankAgents(candidates, 1, {
    requiredWeightKg,
    fragileRequired,
    checkinRequired,
  });
...
// Optional helper for testing/admin bootstrapping of support agents
    ...
    await runQuery(
      `INSERT INTO support_agents (name, phone, status, current_user_id, max_weight_kg, supports_fragile, supports_checkin)
       ...
       ON DUPLICATE KEY UPDATE
         ...
         supports_fragile = VALUES(supports_fragile),
...
// Agent login heartbeat: mark available on login
    ...
    await runQuery(
      `INSERT INTO support_agents (name, phone, status, current_user_id, latitude, longitude, h3_index, vehicle_type, max_weight_kg, supports_fragile, supports_checkin, location_updated_at)
       ...
       ON DUPLICATE KEY UPDATE
         ...
         h3_index = COALESCE(VALUES(h3_index), h3_index),
      ...
      [
        ...
        vType,
        maxWeight,
        fragile === null ? null : (fragile ? 1 : 0),
        checkin === null ? null : (checkin ? 1 : 0),
        lat,
        lng,
      ]
    );
...
// Agent sends live location updates used for nearby matching
  ...
  try {
    if (agentId) {
      ...
      if (!lookupPhone && lookupName) {
        ...
        if (phoneByName.length) {
          lookupPhone = phoneByName[0].phone || null;
        }
      }
    }
...
// Recommend best nearby agents using H3 + greedy score + load balancing
router.post("/recommend-nearby", verifyToken, async (req, res) => {
  ...
  try {
    ...
    if (fragileRequired === null) {
      fragileRequired = Boolean(latestBooking?.is_fragile);
    }
    if (checkinRequired === null) {
      checkinRequired = Boolean(latestBooking?.is_checkin);
    }
    ...
    const recommended = greedyRankAgents(candidates, limit, {
      requiredWeightKg,
      fragileRequired,
      checkinRequired,
    });
    ...
    return res.json({
      ...
      strategy: {
        ...
        algorithm: "greedy-load-balanced",
      },
      userLocation: {
        latitude: userLat,
        longitude: userLng,
...
// Auto-assign best available agent using H3 + greedy + load balancing + booking constraints
router.post("/assign-best-agent", verifyToken, async (req, res) => {
  try {
    ...
    const mockReq = {
      ...
      body: {
        ...req.body,
        limit: Math.max(3, Number(req.body?.limit || 5)),
      },
    };

    // Reuse recommendation logic by invoking the same query path inline
    const limit = Math.max(1, Math.min(Number(mockReq.body?.limit || 5), 10));
    let userLat = toNumberOrNull(mockReq.body?.latitude);
    let userLng = toNumberOrNull(mockReq.body?.longitude);
    let requiredWeightKg = parseWeightKg(mockReq.body?.bagWeightKg ?? mockReq.body?.bagWeight);
    let fragileRequired = toBoolean(mockReq.body?.isFragile);
    let checkinRequired = toBoolean(mockReq.body?.isCheckin);
    ...
    const ranked = greedyRankAgents(candidates, limit, {
      requiredWeightKg,
      fragileRequired,
      checkinRequired,
    });
    ...
    for (const candidate of ranked) {
      if (candidate.fit === false) {
        continue;
      }

      const updateResult = await runQuery(
        `UPDATE support_agents
         SET status = 'busy', current_user_id = ?, last_assigned_at = CURRENT_TIMESTAMP
      ...
      const sessionResult = await runQuery(
        `INSERT INTO agent_sessions (user_id, agent_id, status, start_time)
    ...
    if (!assignedSession) {
      ...
      return res.json({
        ...
        assigned: false,
        queued: true,
        message: "No fit available agent right now. Added to queue.",
        ranked,
      });
    }
    ...
    return res.json({
      ...
      assigned: true,
      queued: false,
      message: "Best-fit agent assigned",
...
router.post("/request-agent", verifyToken, async (req, res) => {
  ...
  try {
    ...
    // Always queue first so agents can explicitly Accept/Decline from their dashboard.
    await runQuery(
...
// Agent inbox: list waiting requests and active sessions for the agent UI
  ...
  try {
    ...
    if (currentAgentId) {
      ...
      if (agentRows.length) {
        ...
        if (lat !== null && lng !== null) {
          currentAgentCoords = { lat, lng };
        }
      }
    }

    const waitingRows = await runQuery(
      `SELECT q.queue_id, q.user_id, q.booking_id, q.preferred_agent_id, q.declined_agent_ids, q.status, q.requested_at,
              u.name, u.phone,
              b.pickup_time, b.departure_date, b.bag_count, b.bag_weight, b.pickup_address,
              b.pickup_latitude, b.pickup_longitude,
              b.drop_address, b.airline_name, b.flight_number, b.assignment_due_at
       FROM agent_queue q
    ...
    for (const row of waitingRows) {
      ...
      if (!targetAgentId && row.booking_id) {
        ...
        if (targetAgentId) {
          await runQuery(
            `UPDATE agent_queue
             SET preferred_agent_id = ?, updated_at = CURRENT_TIMESTAMP
             WHERE queue_id = ?`,
            [targetAgentId, row.queue_id]
          );
        }
      }

      if (currentAgentId && targetAgentId && targetAgentId !== currentAgentId) {
        continue;
      }

      const pickupLat = toNumberOrNull(row.pickup_latitude);
      const pickupLng = toNumberOrNull(row.pickup_longitude);
      const distanceKm = currentAgentCoords && pickupLat !== null && pickupLng !== null
        ? Number(haversineKm(currentAgentCoords.lat, currentAgentCoords.lng, pickupLat, pickupLng).toFixed(2))
        : null;

      const etaMinutes = distanceKm !== null
        ? Math.max(1, Math.ceil((distanceKm / DEFAULT_CITY_SPEED_KMPH) * 60) + PICKUP_PREP_BUFFER_MIN)
        : null;

      const targetPickupAt = toDateOrNull(row.assignment_due_at) || parsePickupDateTime(row.departure_date, row.pickup_time);
      const leaveByAt = targetPickupAt && etaMinutes !== null
        ? new Date(targetPickupAt.getTime() - etaMinutes * 60 * 1000)
        : null;

      const urgencyMinutes = leaveByAt
        ? Math.round((leaveByAt.getTime() - Date.now()) / 60000)
        : null;
    ...
    visibleWaiting.sort((a, b) => {
      const aUrgency = Number.isFinite(a.urgency_minutes)
        ? a.urgency_minutes
        : Number.POSITIVE_INFINITY;
      const bUrgency = Number.isFinite(b.urgency_minutes)
        ? b.urgency_minutes
        : Number.POSITIVE_INFINITY;
      if (aUrgency !== bUrgency) return aUrgency - bUrgency;

      const aEta = Number.isFinite(a.eta_minutes)
        ? a.eta_minutes
        : Number.POSITIVE_INFINITY;
      const bEta = Number.isFinite(b.eta_minutes)
        ? b.eta_minutes
        : Number.POSITIVE_INFINITY;
...
// Agent responds to a queued request by accepting or declining it
router.post("/respond-request", async (req, res) => {
  ...
  try {
    ...
    if (resolvedAgentId) {
      const agentRows = await runQuery(
        "SELECT agent_id FROM support_agents WHERE agent_id = ? LIMIT 1",
        [resolvedAgentId]
      );
      if (!agentRows.length) {
    ...
    if (!resolvedAgentId && agentPhone) {
      await runQuery(
        `INSERT INTO support_agents (name, phone, status, current_user_id)
         VALUES (?, ?, 'available', NULL)
      ...
      const rows = await runQuery(
        "SELECT agent_id FROM support_agents WHERE phone = ? LIMIT 1",
        [agentPhone]
      );
    ...
    const queueRows = await runQuery(
      `SELECT q.queue_id, q.user_id, q.booking_id, q.preferred_agent_id, q.status, u.name, u.phone,
    ...
    if (action === 'decline') {
      ...
      return res.json({
        success: true,
        action: 'declined',
        reassigned: Boolean(nextAgentId),
        nextPreferredAgentId: nextAgentId,
        message: nextAgentId
    ...
    const existingSessionRows = await runQuery(
      `SELECT session_id, agent_id, start_time
       FROM agent_sessions
    ...
    if (existingSessionRows.length > 0) {
      // Request is already in progress; clear queue entry and return success.
      await runQuery(
        "DELETE FROM agent_queue WHERE queue_id = ? AND status = 'waiting'",
        [queueId]
      );
      ...
      return res.json({
        ...
        session: {
          sessionId: existingSessionRows[0].session_id,
  ...
  try {
    ...
    if (activeRows.length) {
      return res.json({
        ...
        queued: false,
        session: {
          sessionId: activeRows[0].session_id,
          startTime: activeRows[0].start_time,
          agent: {
            agentId: activeRows[0].agent_id,
            name: activeRows[0].name,
            phone: activeRows[0].phone,
    ...
    const queueRows = await runQuery(
      "SELECT queue_id, requested_at FROM agent_queue WHERE user_id = ? AND status = 'waiting' ORDER BY requested_at ASC LIMIT 1",
      [user.id]
    );
    ...
    if (queueRows.length) {
      return res.json({
        ...
        queued: true,
        queue: {
          queueId: queueRows[0].queue_id,
          requestedAt: queueRows[0].requested_at,
        },
      });
    }
...
// Complete the active session and free agent
