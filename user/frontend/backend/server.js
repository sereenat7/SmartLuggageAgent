...
// File-based logger
const logFile = path.join(logsDir, `requests-${Date.now()}.log`);
...
app.use((req, res, next) => {
    const logMsg = `${req.method.toUpperCase()} ${req.url}`;
    console.log(`\n🔵 [${new Date().toISOString()}] ${logMsg}`);
    console.log('Headers:', JSON.stringify(req.headers, null, 2));
    console.log('Body:', JSON.stringify(req.body, null, 2));
    fileLog(`REQUEST: ${logMsg}`);
    fileLog(`HEADERS: ${JSON.stringify(req.headers)}`);
    fileLog(`BODY: ${JSON.stringify(req.body)}`);
    next();
});
...
app.post("/api/payment/test", (req, res) => {
  fileLog("💳 TEST PAYMENT ENDPOINT CALLED");
